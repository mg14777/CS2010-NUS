...
class Bleeding {
  ...
  /**
  Initialisation:
    During initialisation path_table[destination][j] = 0 for all j (remaining hops) since cost of reaching destination from destination is 0
    for any number of remaining hops

    path_table[i][0] = MAXWEIGHT (except i=destination) since path cost of reaching destination from any other vertex with 0 remaining hops is 
    infinity

    Rest of the elements are -1 since they haven't been calculated yet
  **/
  ...
  /**
  Approach:
  Divide the problem into individual sub problems based on following idea:
    Shortest path from source to destination is same as Min(shortest path from neighbour to destination + weight of that edge) for all neighbours

  There can be different shortest paths for the same pair of vertices depending on number of hops allowed which is why we store each of those
  distances for a particular vertex leading to a 2D memo table (path_table)

  The element we need to find is path_table[source][allowed_hops - 1] since the source vertex always takes up 1 hop/junction
  **/
